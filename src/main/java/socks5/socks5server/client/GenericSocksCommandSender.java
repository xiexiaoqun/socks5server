...
package socks5.socks5server.client;

import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import socks5.socks5server.common.AddressType;
import socks5.socks5server.common.ProtocolErrorException;
import socks5.socks5server.common.SocksCommand;
import socks5.socks5server.common.SocksException;
import socks5.socks5server.utils.LogMessageBuilder;
import socks5.socks5server.utils.LogMessageBuilder.MsgType;
import socks5.socks5server.utils.UnsignedByte;
...
public class GenericSocksCommandSender implements SocksCommandSender {
    ...
    public CommandReplyMessage send(Socket socket, SocksCommand command, SocketAddress
        ...
        final boolean haveHostName = address.getHostName() != null;
        final byte[] bytesOfAddress = haveHostName ? address.getHostName().getBytes() : address.getAddress().getAddress();
        ...
        int offset = 0;
        if (haveHostName) {
            addressType = ATYPE_DOMAINNAME;
            bufferSent = new byte[6 + ADDRESS_LENGTH + 1];
            bufferSent[4]=(byte) bytesOfAddress.length;
            offset = 1;
        } else if (ADDRESS_LENGTH == LENGTH_OF_IPV4) {
        ...
        System.arraycopy(bytesOfAddress, 0, bufferSent, 4 + offset, ADDRESS_LENGTH);// copy address bytes
        bufferSent[4 + offset + ADDRESS_LENGTH] = (byte) ((port & 0xff00) >> 8);
        bufferSent[5 + offset + ADDRESS_LENGTH] = (byte) (port & 0xff);
